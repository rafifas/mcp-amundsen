...
class TableApiClient(AmundsenApiClient):
    ...
    def get_table_url(self, database: str, schema_name: str, table_name: str, cluster: str = "gold") -> str:
        """
        Constructs the base table URL.
        """
        return f"{self.BASE_URL}/table/{database}://{cluster}.{schema_name}/{table_name}"

    def get_metadata(self, database: str, schema_name: str, table_name: str, cluster: str = "gold") -> dict[str, Any]:
        """
        Fetches metadata for a specific table.
        """
        url = self.get_table_url(database, schema_name, table_name, cluster)
        try:
    ...
    def get_lineage(
        ...
        base_url = self.get_table_url(database, schema_name, table_name, cluster)
        url = f"{base_url}/lineage?depth={depth}&direction={direction}"
        return self.make_request(url)

    def get_dashboards(self, database: str, schema_name: str, table_name: str, cluster: str = "gold") -> dict[str, Any]:
        """
    ...
    def questions(
        ...
        resp = self.get_dashboards(database, schema, table, cluster)
    ...
    def question_detail(
        ...
        questions = self.questions(database, schema, table, dashboard_name, cluster=cluster)
        try:
